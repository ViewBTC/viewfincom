...
class suxingme_hotpost extends WP_Widget {
    ...
    function suxingme_hotpost() {
        $widget_ops = array('description' => '默认显示30天内，评论数最多的图文列表');
        $this->__construct('suxingme_hotpost', '热评文章 ', $widget_ops);
    }
				...
				while ( $hot_posts->have_posts() ) :$hot_posts->the_post(); ?>
					<li>
						<a class="img" href="<?php the_permalink(); ?>"	title="<?php the_title(); ?>">
							...
							<div class="title">
								<div class="entry-meta"><span><i class="icon-comment-inv-alt2"></i> <?php echo get_comments_number('0','1','%');?></span><span><i class="icon-calendar-1"></i> <?php the_time('Y-n-j') ?></span></div>
